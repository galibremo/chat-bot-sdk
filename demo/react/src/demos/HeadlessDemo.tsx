  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--gray-900)' }}>TypeTechIT Assistant</div>
...
function EmptyState() {
  return (
    <div style={{
      ...
      color: 'var(--gray-500)', paddingTop: 40,
    }}>
      <div style={{
        width: 56, height: 56,
      ...
      <p style={{ fontWeight: 600, color: 'var(--gray-700)' }}>Start a conversation</p>
      <p style={{ fontSize: 13 }}>Type a message below to chat with the assistant.</p>
    </div>
  );
}
    ...
    <div>
      <div style={{ marginBottom: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--gray-900)' }}>Custom UI with <code>useChatbot()</code></h2>
        </div>
        <p style={{ color: 'var(--gray-500)', fontSize: 14 }}>
          Use the <code>useChatbot()</code> hook to build any UI you want — complete control
          over layout, styling, and behaviour. No Shadow DOM, no built-in widget.
        </p>
      </div>
      ...
      {!API_KEY ? (
        <div style={{
          ...
          borderRadius: 'var(--radius)', padding: '16px 20px', fontSize: 13, fontWeight: 500,
          marginBottom: 32,
        }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'start' }}>
        {/* Live UI */}
        <div>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--gray-700)', marginBottom: 12 }}>Live preview</h3>
          {API_KEY ? (
            <ChatbotProvider
              apiKey={API_KEY}
              apiBaseUrl={BASE_URL}
        ...
        {/* Code */}
        <div>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--gray-700)', marginBottom: 12 }}>Source code</h3>
          <pre style={{ fontSize: 12, lineHeight: 1.65, borderRadius: 'var(--radius)' }}><code>{CODE_SNIPPET}</code></pre>

          <div style={{ marginTop: 20, background: '#fff', border: '1px solid var(--gray-200)', borderRadius: 'var(--radius)', padding: 20 }}>
            <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>Hook return values</h4>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <tbody>
                {HOOK_FIELDS.map(([name, type, desc]) => (
                  <tr key={name} style={{ borderBottom: '1px solid var(--gray-100)' }}>
                    <td style={{ padding: '7px 0', fontFamily: 'monospace' }}><code>{name}</code></td>
                    <td style={{ padding: '7px 8px', color: 'var(--gray-500)', fontFamily: 'monospace', fontSize: 11 }}>{type}</td>
                    <td style={{ padding: '7px 0', color: 'var(--gray-700)', paddingLeft: 8 }}>{desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
